...
tt_low_g_extra <-183 # 6 months
tt_high_g_extra <- 365 # 1 year
...
results_gastropods_extra <- monte_carlo_production(SS_av_gastro, ss_uncertainty =  0.25, tt_low =  tt_low_g_extra, tt_high = tt_high_g_extra, n_simulations = 100000, confidence_level = 0.95 )
...
p_cocco_ss <- ggplot(data.frame(prod), aes(x = prod)) +
  ...
  labs(title = "Production coccolithophores",
...
p_gastro_ss <- ggplot(data.frame(prod), aes(x = prod)) +
  ...
  geom_vline(xintercept = ci_l, color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = ci_u, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Production gastropods",
       x = "Production [mg/m2/day]", y = "Frequency") +
  theme_minimal()+
  theme(
    axis.title.y = element_text(size = 12),
...
prod <- results_gastropods_extra$production_samples
ci_l <- results_gastropods_extra$ci_lower
ci_u <- results_gastropods_extra$ci_upper

# Plot histogram of production results with CI lines
p_gastro_ss_extra <- ggplot(data.frame(prod), aes(x = prod)) +
  geom_histogram(bins = 50, fill = "steelblue", alpha = 0.7, color = "black") +
  geom_vline(xintercept = ci_l, color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = ci_u, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Production gastropods long turnover",
       x = "Production [mg/m2/day]", y = "Frequency") +
  theme_minimal()+
  theme(
    axis.title.y = element_text(size = 12),
...
p_foram_ss <- ggplot(data.frame(prod), aes(x = prod)) +
  ...
  geom_vline(xintercept = ci_l, color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = ci_u, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Production foraminifera ",
       x = "Production [mg/m2/day]", y = "Frequency") +
  theme_minimal()+
  theme(
...
# Arrange plots
finalplot <- ggarrange(p_gastro_ss, p_gastro_ss_extra, p_foram_ss, p_cocco_ss,
          labels = c("a", "b", "c", "d"),
          ncol = 2, nrow = 2,
